...
public class GameSystem {
	...
	//Fill your code here
		if (piece instanceof MotherShip) {
			if (attack) {
				Position position = piece.attackTargetPosition(actionPattern);
				removeOtherPieces(piece, position);
			} else {
			...
			removeOtherPieces(piece, piece.getCurrentPosition());
			if (removeOtherPieces(piece, piece.getCurrentPosition())) {
				((Fighter) piece).setPromoted(true);
			}
		}
